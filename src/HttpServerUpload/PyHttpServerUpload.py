...
def showTips():
    ...
    try:
        ...
        print '-------->> Please visit files or dirs use Chrome Browser:     http://' + GetWanIp().getip() + ':' + str(
            port)
        print "-------->> Also, You can wget download the file:    " + echoRed(
            "wget http://" + GetWanIp().getip() + ':' + str(port) + "/" + file_name)
    except Exception, e:
        print 'You have not give a filename, plase use Chrome Browser:     http://' + GetWanIp().getip() + ':' + str(
            port)
